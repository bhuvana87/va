...
function DrawRectangle(x, y, w, h) {
  ...
  selxcoord = element.attr('x');
  selycoord = element.attr('y');
...
(function() {
  ...
  function vjsAnnotation_(options) {
    ...
    function initialVideoFinished(event) {
      ...
      player.rangeslider.rstb.on('mousedown', function() {
  ...
  // Prevent text edit cursor while dragging in webkit browsers
      ...
      $("#svg_paper").mouseup(function (e) {
          ...
          selwidth = BBox.width;
          selheight = BBox.height;
          if (BBox.width == 0 && BBox.height == 0) rect.remove();
      });               

       // Clear canvas
        $(".annotator-cancel").click(function (e) {
            $('#svg_paper').css('display','none');
        });
  ...
  // Delete rect
        ...
        $(".annotator-save").click(function (e) {
            $('#svg_paper').css('display','none');
        });
    ...
    showAnnotation: function(annotation) {
      ...
      if (isVideo) {
        var start = annotation.rangeTime.start,
          ...
          //DrawRectangle(annotation.target.xcoord, annotation.target.ycoord, annotation.target.width, annotation.target.height)
        ...
        //$('#svg_paper').css('display','block');
        var rect = paper.rect(annotation.target.xcoord, annotation.target.ycoord, annotation.target.width, annotation.target.height);
  ...
  videojs.BigNewAnnotation.prototype.createEl = function() {
    return videojs.Button.prototype.createEl.call(this, 'div', {
      ...
      innerHTML: '<div class="vjs-big-menu-button vjs-control">Add Annotation</div>',
  ...
  videojs.AnStat.prototype.paintCanvas = function() {
    ...
    for (var index in points) {
      var p = points[index],
        x1 = lastSe * w.X,
        y1 = (maxEn - lastEn) * w.Y, // Old Point
        x2 = p.second * w.X,
        y2 = (maxEn - p.entries) * w.Y; // New Point
      //new line
...
Annotator.Plugin.VideoJS = (function(_super) {
  ...
  VideoJS.prototype.pluginSubmit = function(field, annotation) {
    ...
    if (this.EditVideoAn()) {
      ...
      annotation.annotationContent = annotation.annotationContent || {};
      annotation.rangeTime.start = time.start || 0; // - rangeTime.start
      annotation.rangeTime.end = time.end || 0; // - rangeTime.end
     // annotation.swidth = selwidth;
      //annotation.sheight = selheight;
      annotation.annotationContent.xcoord = selxcoord;
      annotation.annotationContent.ycoord = selycoord;
      annotation.annotationContent.sheight = selheight;
      annotation.annotationContent.swidth = selwidth;
      annotation.annotationContent.ranges = annotation.ranges;
      

      annotation.annotationContent.updated = new Date().toISOString(); // - updated
      if (typeof annotation.annotationContent.created == 'undefined')
        annotation.annotationContent.created = annotation.annotationContent.updated; // - created
      ...
      if (typeof annotation.annotationContent.media == 'undefined')
        annotation.annotationContent.media = "text"; // - media
      annotation.annotationContent.updated = new Date().toISOString(); // - updated
      if (typeof annotation.annotationContent.created == 'undefined')
        annotation.annotationContent.created = annotation.annotationContent.updated; // - created
    }
    return annotation.annotationContent.media;
  };
